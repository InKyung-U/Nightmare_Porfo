...
CSceneScript_02::CSceneScript_02()
	: CScript((int)SCRIPT_TYPE::SCENESCRIPT_02)
	, mainCam()
{
	m_pScene01 = new CScene;
	CSceneMgr::GetInst()->ChangeSceneNumber(2);

	wstring strTileFolder = CPathMgr::GetInst()->GetContentPath();
	strTileFolder = strTileFolder + L"scene\\stage02.scene";
	...
	//CPrefab* prefab1 = CResMgr::GetInst()->Load<CPrefab>(L"prefab\\Skeleton_Lizard_Re.pref", L"prefab\\Skeleton_Lizard_Re.pref", false).Get();
	...
	monster->Transform()->SetRelativePos(Vec3(-400, 230, 0));
	pLoadScene->AddObject(monster, L"Monster");

	monster = prefab2->Instantiate();
	monster->Transform()->SetRelativePos(Vec3(300, 230, 0));
	pLoadScene->AddObject(monster, L"Monster");

	//monster = prefab1->Instantiate();
	//monster->Transform()->SetRelativePos(Vec3(0, 1000, 0));
	//pLoadScene->AddObject(monster, L"Monster");
	...
	monster = prefab3->Instantiate();
	monster->Transform()->SetRelativePos(Vec3(500, -350, 0));
	pLoadScene->AddObject(monster, L"Monster");
	...
	pPlayerObject->Transform()->SetRelativePos(-446.f, 227.f, 200.f);
	...
	pCamObj->Camera()->SetCameraIndex(0);
	...
	ChangeScene->Transform()->SetRelativePos(1500, -350, 0);
...
void CSceneScript_02::start()
{
}

void CSceneScript_02::update()
{
	//mainCam->Camera()->SetCameraAsMain();
}
