...
private:
	...
	vector<wstring>		m_Atlas;
	vector<wstring>		m_Animation;
	CAnimator2D*		m_pSavedAnimator;
public:
	void init();
	void update();
...
public:
	...
	void AddAtlas(const wstring& _strKey) { m_Atlas.push_back(_strKey); }
	const vector<wstring>& GetAtlasList() { return m_Atlas; }

	void CreateAnimation(const wstring& _strName, Ptr<CTexture> _pAtlas, vector<AnimToolData> _vec);
	void AddAnimationList(const wstring& _strKey) { m_Animation.push_back(_strKey); }
	const vector<wstring>& GetAnimationList() { return m_Animation; }
	CAnimator2D* GetSavedAnimator() { return m_pSavedAnimator; }
