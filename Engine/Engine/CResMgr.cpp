...
void CResMgr::CreateAnimation(const wstring& _strName, Ptr<CTexture> _pAtlas, vector<AnimToolData> _vec)
{
	CAnimator2D* pAnim = new CAnimator2D;

	pAnim->SetName(_strName);
	pAnim->CreateAnim(_strName, _pAtlas, _vec);

	AddAnimationList(_strName);
	m_pSavedAnimator = pAnim;
	delete pAnim;
}
