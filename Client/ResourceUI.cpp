	...
	, m_pObject(nullptr)
{
	...
	// Drag and Drop Deletage 등록
	m_TreeUI->SetDragAndDropDelegate(this, (DRAG_DROP)&ResourceUI::ItemDragAndDrop);
	m_TreeUI->UseDragDropToSceneOutliner(true);
...
void ResourceUI::render_update()
{
	...
	//static char newPrefabName[255] = {};
	//ImGui::InputText("##NewPrefabName", newPrefabName, IM_ARRAYSIZE(newPrefabName));
	//if (ImGui::Button("Add Frefab"))
	//{
	//	if (newPrefabName[0] == '\0')
	//	{
	//		return;
	//	}
	//	else // 이름이 있으면 프리펩에 추가 후 프리펩 이름 지움
	//	{
	//		string sPrefabName(newPrefabName);
	//		wstring wsPrefabName = {};
	//		wsPrefabName.assign(sPrefabName.begin(), sPrefabName.end());
	//
	//		wstring makedPrefabName = L"prefab\\" + wsPrefabName + L".pref";
	//		if (nullptr == CResMgr::GetInst()->FindRes<CPrefab>(makedPrefabName))
	//		{
	//			m_pObject = new CGameObject;
	//			m_pPrefab = new CPrefab;
	//			m_pPrefab->SetProto(m_pObject->Clone());
	//			CResMgr::GetInst()->AddRes<CPrefab>(makedPrefabName, m_pPrefab);
	//
	//			delete m_pObject;
	//			memset(newPrefabName,'\0', 255);
	//			Renew();
	//		}
	//	}
	//}
...
void ResourceUI::ItemDragAndDrop(DWORD_PTR _dwDrag, DWORD_PTR _dwDrop)
{
	CRes* pDrag = (CRes*)_dwDrag;
	UI* pDrop = (UI*)_dwDrop;

	// 현재 클릭한 노드가 Prefab 인지 확인
	if (pDrag->GetResType() != RES_TYPE::PREFAB)
		return;
	
	if (nullptr == pDrop)
		return;
	// 프리펩을 SceneOutLine 으로 드래그 하면 오브젝트 추가
	if (pDrop->GetTitle() == "SceneOutliner")
	{
		int i = 0;
	}
