...
void TileMapEditor::render_update()
{
    ...
    const ImVec2 zoomed_tilesize = ImVec2{ m_pTargetTileMap->GetSlicePixelSize().x / zoom,m_pTargetTileMap->GetSlicePixelSize().y / zoom };
    ...
    // 타일맵 그리기
    CTexture* pAtlasTexture = m_pTargetTileMap->GetAtlas();
        ...
        if (mouse_pos_in_canvas.x > 0 && mouse_pos_in_canvas.x < vZoomedTileMapSize.x && mouse_pos_in_canvas.y > 0 && mouse_pos_in_canvas.y < vZoomedTileMapSize.y)
        {
                ...
                ImGui::IsMouseDragging(ImGuiMouseButton_Left))
            {
                ...
                if (iSelectedIdx != -1)
                {
                    ...
                    int iSelected_col = m_pLSEditor->GetCurTileIdx() % 10;
                    int iSelected_row = m_pLSEditor->GetCurTileIdx() / 10;
